...
import { Save, Loader2, Key, Database, Image as ImageIcon } from 'lucide-react';
import { cn } from '../lib/utils';

// Loose typing for the config as it's dynamic
interface Config {
    tmdb?: { api_key: string };
    omdb?: { api_key: string };
...
export function SettingsView() {
    ...
    return (
        <div className="p-8 glass-panel rounded-xl max-w-4xl mx-auto w-full mt-4 flex flex-col gap-8">
            <div className="flex justify-between items-center">
                <div>
                    <h3 className="text-2xl font-bold flex items-center gap-3">
                        Config Manager
                    </h3>
                    <p className="text-sm text-secondary">Manage API keys and scraper preferences.</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className="px-6 py-2 bg-primary hover:bg-primary/90 text-white rounded-lg font-medium flex items-center gap-2 transition-all disabled:opacity-50"
                >
